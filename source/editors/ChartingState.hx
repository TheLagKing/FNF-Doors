...
import objects.StrumNote.VortexStrumNote;
...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		'All Sing',
		'Guidance Note',
		'Alt Animation+Guidance Note'
	];
	private var noteTypeIntMap:Map<Int, String> = new Map<Int, String>();
	private var noteTypeMap:Map<String, Null<Int>> = new Map<String, Null<Int>>();

	public static var DEFAULTCHARLIST:Array<String> = [
		'BF',
		'GF',
		'Dad'
	];
	private var charIntMap:Map<Int, String> = new Map<Int, String>();
	private var charMap:Map<String, Null<Int>> = new Map<String, Null<Int>>();
	...
	[
		...
		['Tween Property', "Value 1: Variable name\nValue 2: start;end;duration;easing"],
		
		//camera
		['cameraFlashHud', "Value 1: color\nValue 2: duration"],
		['cameraFlashGame', "Value 1: color\nValue 2: duration"],
		["cameraFadeGame", "Value1: color\nValue2: duration"],
		["cameraFadeHud", "Value1: color\nValue2: duration."],
		["endFadeGame", "Value1: color\nValue2: duration."],
		["endFadeHud", "Value1: color\nValue2: duration."],
		["cameraFreeMove", "Activate the new camera system. \nTHIS IS DANGEROUS IF YOU DON'T KNOW WHAT YOU'RE DOING."],
		["stopCameraFreeMove", "Deactivate the new camera system"],
		["focusCameraOnCharacter", "0 = bf, 1 = dad, 2 = gf, 3 = 2nd enemy. \n this focuses the camera on the selected character."],
		["addCameraOffset", "Value1 = x value, Value2 = y value. \n Remember to set it to 0 when you're not using this anymore!"],
		["focusCameraOnPoint", "Value1 = x value, Value2 = y value. \n This will override any camera setting and set the camera position \nto the coordinates in value 1 & 2."],
		['Deactivate CamFollow', "Value 1: true/false (true = activate). \nRemember to reactivate it."],
		['remove Cam Actuation', "no i didn't misspell this, don't use it \nIt basically deactivates the camera zooming back into \ncorrect values, for special effects only lol"],
		['zoomIn', "Value 1: Zoom,Add. you can use 'cur' to use the current zoom and then add. \nExample : cur,0.5. \nValue 2: duration,easing. \n0.5,quartOut\ndefault is sineInOut"],
		
		//Additional effects
		['badApple', "Makes the background white and the characters black.\nValue1: none\nValue2: duration"],
		['endBadApple', "remove the bad apple effect.\nValue1: none\nValue2: duration"],
		['Cinematics Bar', "value1: Distance\nValue2: Ease time."],
		["bf pop off section ghost", "no way"],
		["spawn credits", "spawn the motherfucking credits"],
		
		//shaders
		['Chromatic Aberrate', "Value 1: How much to activate the Aberration. \nRecommended value : 0-10. \nValue 2: How much time to activate it."],
		['Pixelated Effect', "Value 1: How much should it be pixelated?\nRecommended value : 0-40.\nValue 2: How fast should it fade in/out"],

		//Song dependant
		['triggerTaiko', "Activate/Deactivate the heartbeat in Figure's songs"],
		
		['bargainJeffAlpha', "Makes the characters opaque in bargain\nSo you can see jeff better when he sings."],
		...
		['toggleSeekPov', "Makes seek pov or remove the pov"],
		["spawnSeek", ""],
		["seekDodge", ""],
		["seekEndSong", "Value 1: duration;ease\nValue 2: True if fakeout, False otherwise"],
		["spawnSeekHand", "Value 1: duration\nValue 2: Pattern : [STATIC/UPDOWN/UP/DOWN]"],
		...
		['Black bars', 'Value 1: make them appear (true/false)\nValue 2: beats'],
		['darken the bg', 'Value 1: make them get dark (true/false)\nValue 2: beats\n\nWARNING: you have to set the things that\nare going to get dark manually'],
		['move insant', 'Value 1 is if it should enable or disable'],

		['set Glitch timer', 'Value 1: time yeah (beats)'],

		['show real time left', 'Value 1: time yeah (beats)'],
		['show fake time left', 'Value 1: time yeah (beats)\nValue 2: fakeTime Extra (real time + value 2)\nWARNING: Value 2 should be in milliseconds'],
	];
	...
	var strumLineNotes:FlxTypedGroup<VortexStrumNote>;
	...
	var curRenderedNoteChar:FlxTypedGroup<HealthIcon>;
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Chart Editor", StringTools.replace(_song.song, '-', ' '), "gameicon");
		#end

		vortex = FlxG.save.data.chart_vortex;
		ignoreWarnings = FlxG.save.data.ignoreWarnings;
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuDesat' + new FlxRandom().int(1, 2)));
		bg.scrollFactor.set();
		bg.color = 0xFF222222;
		add(bg);

		lilStage = new FlxSprite(974, 432).loadGraphic(Paths.image("chartEditor/lilStage"));
		lilStage.scrollFactor.set();
		add(lilStage);

		lilBf = new FlxSprite(974, 432).loadGraphic(Paths.image("chartEditor/lilBf"), true, 300, 256);
		...
		lilOpp = new FlxSprite(974, 432).loadGraphic(Paths.image("chartEditor/lilOpp"), true, 300, 256);
		...
		curRenderedNoteChar = new FlxTypedGroup<HealthIcon>();
		...
		strumLineNotes = new FlxTypedGroup<VortexStrumNote>();
		for (i in 0...8){
			var note:VortexStrumNote = new VortexStrumNote(GRID_SIZE * (i+1), strumLine.y, i % 4, 0, (i >= 4)?1:0);
		...
		\nCredits to Rozebud and FPS+ for the lil dudes!!!";
		...
		zoomTxt = new FlxText(10, 50, 0, "Zoom: 1 / 1", 16);
	...
	var lilBuddiesBox:FlxUICheckBox = null;
	var check_warnings:FlxUICheckBox = null;
	var playSoundBf:FlxUICheckBox = null;
	var playSoundDad:FlxUICheckBox = null;
	var UI_songTitle:FlxUIInputText;
	var noteSkinInputText:FlxUIInputText;
	var oppSkinInputText:FlxUIInputText;
	var noteSplashesInputText:FlxUIInputText;
	var stageDropDown:FlxUIDropDownMenuCustom;
	var characterDropDownArr:Array<FlxUIDropDownMenuCustom> = [];
	var tab_group_song:FlxUI;
	function addSongUI():Void
	{
		if(tab_group_song != null){
			tab_group_song.kill();
		}
		...
		var numberOfChars:FlxUINumericStepper = new FlxUINumericStepper(10 + 100, stepperSpeed.y, 1, 3, 3, 12, 0);
		//12 is an arbitrary limit, just put whatever, but be careful because lag can and will appear if too many characters are in.
		//Also some buttons may not be possible to access if there's too many characters lmao
		numberOfChars.value = _song.characters.length;
		numberOfChars.name = 'num_char';
		blockPressWhileTypingOnStepper.push(numberOfChars);

		if(characterDropDownArr != []){
			var i = characterDropDownArr.length - 1;
			while(i >= 0){
				tab_group_song.remove(characterDropDownArr[i]);
				tab_group_song.remove(new FlxText(characterDropDownArr[i].x, characterDropDownArr[i].y - 15, 0, '${characterDropDownArr[i].selectedLabel}:'));
				i--;
			}
		}
		characterDropDownArr = [];
		for(i in 0..._song.characters.length){
			var characterDropDown = new FlxUIDropDownMenuCustom(10, stepperSpeed.y + 45 + (i * 40), FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
				{
					_song.characters[i] = characters[Std.parseInt(character)];
					updateHeads();
					addNoteUI();
				});
			characterDropDown.selectedLabel = _song.characters[i];
			blockPressWhileScrolling.push(characterDropDown);
			characterDropDownArr.push(characterDropDown);
		}
		...
		stageDropDown = new FlxUIDropDownMenuCustom(characterDropDownArr[0].x + 140, characterDropDownArr[0].y, FlxUIDropDownMenuCustom.makeStrIdLabelArray(stages, true), function(character:String)
		{
		...
		var skin2 = PlayState.SONG.opponentArrowSkin;
		
		if(skin == null) skin = '';
		if(skin2 == null) skin2 = '';
		
		noteSkinInputText = new FlxUIInputText(characterDropDownArr[2].x, characterDropDownArr[2].y + 50, 150, skin, 8);
		oppSkinInputText = new FlxUIInputText(characterDropDownArr[2].x + 160, characterDropDownArr[2].y + 50, 120, skin2, 8);
		
		blockPressWhileTypingOn.push(noteSkinInputText);
		blockPressWhileTypingOn.push(oppSkinInputText);
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 20, 'Change Notes', function() {
			_song.opponentArrowSkin = oppSkinInputText.text;
		...
		*/
		...
		tab_group_song.add(numberOfChars);
		/*tab_group_song.add(reloadNotesButton);
		tab_group_song.add(noteSkinInputText);
		tab_group_song.add(oppSkinInputText);
		...
		var i = characterDropDownArr.length - 1;
		while(i >= 0){
			tab_group_song.add(characterDropDownArr[i]);
			tab_group_song.add(new FlxText(characterDropDownArr[i].x, characterDropDownArr[i].y - 15, 0, '${characterDropDownArr[i].selectedLabel}:'));
			i--;
		}
		tab_group_song.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 0, 'Stage:'));
		/*
		tab_group_song.add(new FlxText(noteSkinInputText.x, noteSkinInputText.y - 15, 0, 'Player Note Texture:'));
		tab_group_song.add(new FlxText(oppSkinInputText.x, oppSkinInputText.y - 15, 0, 'Opponent Textures:'));
	...
	var charDropDownGroup:Array<FlxUICheckBox> = [];
	var currentType:Int = 0;
	var currentChars:Array<Int> = [];
	var tab_group_note:FlxUI;

	function addNoteUI():Void
	{
		if(tab_group_note != null){
			tab_group_note.kill();
		}
		...
		if(charDropDownGroup != []){
			var i = charDropDownGroup.length - 1;
			while(i >= 0){
				tab_group_song.remove(charDropDownGroup[i]);
				i--;
			}
		}
		...
		charDropDownGroup = [];
		for (i in 0..._song.characters.length){
			var noteCharacterCheckBox = new FlxUICheckBox(noteTypeDropDown.x + 140, 105 + (i * 40), null, null, 'Applies to character ${_song.characters[i]}', 100);
			noteCharacterCheckBox.callback = function()
				{
					noteCharacterCheckBox.checked ? currentChars.push(i):currentChars.remove(i);
					if(curSelectedNote != null && curSelectedNote[1] > -1) {
						curSelectedNote[4] = currentChars.copy();
					}
					updateGrid();
				};
			charDropDownGroup.push(noteCharacterCheckBox);
		}
		...
		var i = charDropDownGroup.length - 1;
		while(i >= 0){
			tab_group_note.add(charDropDownGroup[i]);
			i--;
		}
	...
	function addEventsUI():Void
	{
		...
		descText.setFormat(FONT, 24, 0xFFFEDEBF);
	...
	var hasHeartbeat:FlxUICheckBox;
	...
	function addChartingUI() {
		...
		hasHeartbeat = new FlxUICheckBox(130, 200, null, null, "Song has Heartbeat ?", 100);
		hasHeartbeat.checked = _song.hasHeartbeat;

		hasHeartbeat.callback = function()
		{
			_song.hasHeartbeat = hasHeartbeat.checked;
			reloadGridLayer();
		};
		...
		tab_group_chart.add(hasHeartbeat);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'num_char'){
				while(_song.characters.length != nums.value){
					nums.value > _song.characters.length ? _song.characters.push('') : _song.characters.pop();
				}
				addSongUI();
				addNoteUI();
			}
	...
	override function update(elapsed:Float)
	{
		...
		currentChars = [];
		for(i in 0...charDropDownGroup.length){
			var c = charDropDownGroup[i];
			if(c.checked) currentChars.push(i);
		}
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(note.isHeartbeat) noteDataToCheck = note.noteData;
					...
					if(strumLineNotes.members[noteDataToCheck]!=null){
						...
						//Something for heartbeat notes here lol idk 
						//trace(noteDataToCheck + " of "+strumLineNotes.members.length);
					}
					...
					if(!playedSound[data]) {
						...
						if(data < 4 && _song.notes[curSec].mustHitSection) isBfNote = true;
						if(data > 3 && !_song.notes[curSec].mustHitSection) isBfNote = true;

						if(!isBfNote) lilOpp.animation.play("" + (note.noteData % 4), true);
						else lilBf.animation.play("" + (note.noteData % 4), true);
					}
				}
			}
		});
	...
	*/
	...
	function reloadGridLayer() {
		...
		{
			if (_song.hasHeartbeat){
				nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 11, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
				nextGridBG.x = -80;
			} else{
		...
		if(foundNextSec)
		{
			...
			if (_song.hasHeartbeat){
				gridBlack = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE * 11), Std.int(nextGridBG.height), FlxColor.BLACK);
			}
			gridBlack.alpha = 0.4;
		...
		if (_song.hasHeartbeat){
			var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + (GRID_SIZE*3)).makeGraphic(2, leHeight, FlxColor.BLACK);
			gridLayer.add(gridBlackLine);
		}
		updateGrid();
	...
	{
		var healthIconP1:String = loadHealthIconFromCharacter(_song.characters[0]);
		var healthIconP2:String = loadHealthIconFromCharacter(_song.characters[2]);
	...
	function updateNoteUI():Void
	{
		if (curSelectedNote != null) {
			if(curSelectedNote[2] != null) {
				...
				if(curSelectedNote[4] != null && curSelectedNote[4] != []){
					currentChars = curSelectedNote[4];
					if(currentChars.length == 0){
						for (i in charDropDownGroup){
							i.checked = false;
						}
					} else {
						for (i in charDropDownGroup) i.checked = false;
						for(j in currentChars){
							charDropDownGroup[j].checked = true;
						}
					}
				}
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[4] != null && i[4] != []){
				var theGay:Null<Int> = i[4][0];
				var theRealGay:Null<String> = _song.characters[theGay];
				var iconName:String = loadHealthIconFromCharacter(theRealGay);
				var daIcon:AttachedIcon = new AttachedIcon(iconName, false);
				daIcon.scrollFactor.set(1, 1);
				daIcon.alpha = 0.5;
				daIcon.setGraphicSize(0, 45);
				daIcon.updateHitbox();
				daIcon.xAdd = -55;
				daIcon.yAdd = -55;
				curRenderedNoteChar.add(daIcon);
				daIcon.theSprTracker = note;
			}
			note.mustPress = _song.notes[curSec].mustHitSection;
			if(i[1] > 3 && i[1] < 8) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var daChars:Array<Int> = i[4];

		var note:Note = new Note(daStrumTime, daNoteInfo > 7? daNoteInfo: daNoteInfo % 4, null, null, true, false, daChars);
		
		//W: Charting screen be like
		var isLeftSide:Bool = (daNoteInfo < 4);
		var isHeartbeatNote:Bool = (daNoteInfo > 7);
		var mustHitThis:Bool = _song.notes[curSec].mustHitSection;
		if(mustHitThis){
			note.isOppNote = !isLeftSide;
		}else{
			note.isOppNote = isLeftSide;
		}
		...
		//W: This is not working as intended
		//note.isOppNote = !_song.notes[curSec].mustHitSection;
		...
		if(daSus != null) { //Common note
			...
			note.noteType = i[3];
			note.assignedChars = i[4];
		} else if (!isHeartbeatNote) { //Event note
		...
		note.x += daNoteInfo > 7 ? Math.floor(-10 * GRID_SIZE) : 0;
		if(isNextSection && _song.notes[curSec].mustHitSection != _song.notes[curSec+1].mustHitSection) {
			// W: pain. I think I figured this out though.
			note.isOppNote = !note.isOppNote;
	...
	function deleteNote(note:Note):Void
	{
		...
		//if(note.isHeartbeat )
		//	trace("whoa mama"); //This does not work here	
		//trace(noteDataToCheck +":"+note.noteData);
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null, chars:Null<Array<Int>> = null):Void
	{
		//curUndoIndex++;
		...
		if (!_song.hasHeartbeat){
			...
			if(noteData > -1)
				...
				{
					_song.notes[curSec].sectionNotes.push([noteStrum, noteData+10, noteSus, noteTypeIntMap.get(daType), daChars]);
					curSelectedNote = _song.notes[curSec].sectionNotes[_song.notes[curSec].sectionNotes.length - 1];
				}
			else
				{
	...
	function getStrumTime(yPos:Float, doZoomCalc:Bool = true):Float
	{
		...
		return FlxMath.remapToRange(yPos, gridBG.y, gridBG.y + gridBG.height * leZoom, 0, (getSectionBeats() * 4) * Conductor.stepCrochet);
	}
...
class AttachedIcon extends HealthIcon
{
	public var theSprTracker:FlxSprite;
	public var xAdd:Float = 0;
	public var yAdd:Float = 0;

	public function new(char, isPlayer) {
		super(char, isPlayer);
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (theSprTracker != null) {
			setPosition(theSprTracker.x + xAdd, theSprTracker.y + yAdd);
			angle = theSprTracker.angle;
			alpha = theSprTracker.alpha - 0.2;
		}
	}
}