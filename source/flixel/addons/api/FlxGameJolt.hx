...
#if GAMEJOLT
typedef TrophyJsonFetch = {
	var success:Dynamic;
	var trophies:Array<TrophyJson>;
}
...
typedef RequestLoader =
{
	...
	var _returnTrophyJson:Null<TrophyJsonFetch>;
	var _getImage:Bool;
	var _batch:Bool;
}
...
class FlxGameJolt
{
	...
	static inline var JSON_RETURN:String = "?format=json";
	 ...
	 * @param	Callback	An optional callback function. Will return a Map<String:String> whose keys and values are equivalent to the key-value pairs returned by GameJolt.
	 */
	public static function fetchTrophy(DataType:Int = 0, ?Callback:Dynamic, ?loaderGroup:String = "trophy", ?isJson:Bool = false):Void
	{
		if (!authenticated)
			return;
		...
		if(isJson){
			tempURL = URL_API + "trophies/" + JSON_RETURN + _idURL;
		}
		...
		sendLoaderRequest(tempURL, Callback, loaderGroup, isJson);
	}
	 ...
	 */
	static function sendLoaderRequest(URLString:String, ?Callback:Dynamic, ?loaderGroup:String = "default", ?isJson:Bool = false):Void
	{
		...
		parseData = function(e:Event)
		{
			...
			if(!isJson){
				...
				var theTrophyJson = cast Json.parse(rawJson).response;
				_loaderGroup._batch = false; //reset
				_loaderGroup._returnTrophyJson = theTrophyJson;

				if (_loaderGroup._callBack != null && _loaderGroup._returnTrophyJson != null && !_verifyAuth) {
					_loaderGroup._callBack(_loaderGroup._returnTrophyJson);
				}
				else if (_verifyAuth) {
					verifyAuthentication(_loaderGroup);
				}
	...
	static function getLoaderGroup(loaderGroup:String)
	{
		...
		{
			var _loaderGroup = {_loader: new URLLoader(), _callBack: null, _returnMap: new Map<String,String>(), _returnTrophyJson: null, _getImage: false, _batch: false};
	...
	{
		#if GAMEJOLT
		var _privateKey = FlxBaseLoader.getLoaded(15325, 32);
		#else
		var _privateKey = "";
		#end
