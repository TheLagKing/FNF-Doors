...
class VarTween extends FlxTween
{
	...
	public function tween(object:Dynamic, properties:Dynamic, duration:Float):VarTween
	{
		...
		if(FlxG.state != null){
			this.stateName = Type.getClassName(Type.getClass(FlxG.state));
			if(verbose)
				trace('tween created on ${this.stateName}');
		}
		start();
		initializeVars();
		return this;
	...
	{
		if(FlxTween.freezeStateName == this.stateName) return;
