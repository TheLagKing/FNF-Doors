...
class FlxGifSpriteAsync extends FlxSprite
{
	/**
	 * The Gif Player.
	 */
	...
	public var callback:FlxSprite->Void = null;
	 ...
	 */
	public function loadGif(Gif:FlxGifAsset, Width:Int = 0, Height:Int = 0, Unique:Bool = false, ?Key:String):FlxGifSpriteAsync
	{
		if (player != null)
		{
			player.dispose(true);
		...
		//player = new GifPlayer(GifDecoder.parseBytesAsync(Assets.getBytes(Std.string(Gif))));

		GifDecoder.parseBytesAsync(Assets.getBytes(Std.string(Gif)),function(a:com.yagp.Gif){
			player = new GifPlayer(a);
