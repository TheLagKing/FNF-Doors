...
class PlayfieldRenderer extends FlxSprite //extending flxsprite just so i can edit draw
{
    ...
    private function addDataToStrum(strumData:NotePositionData, strum:StrumNote)
    {
        ...
        strum.y = strumData.y;
        //strum.z = strumData.z;
        strum.strumSprite.angle = strumData.angle;
        strum.strumSprite.alpha = strumData.alpha;
        strum.strumSprite.scale.x = strumData.scaleX;
        strum.strumSprite.scale.y = strumData.scaleY;
        strum.strumSprite.skew.x = strumData.skewX;
        strum.strumSprite.skew.y = strumData.skewY;
    }
    ...
    private function getNoteCurPos(noteIndex:Int, strumTimeOffset:Float = 0)
    {
        if (notes.members[noteIndex].isSustainNote && ModchartUtil.getDownscroll(instance))
            strumTimeOffset -= Std.int(Conductor.stepCrochet/getCorrectScrollSpeed()) * 2; //psych does this to fix its sustains but that breaks the visuals so basically reverse it back to normal
    ...
    private function drawStrum(noteData:NotePositionData)
    {
        ...
        var thisNotePos = changeX ?
            ModchartUtil.calculatePerspective(new Vector3D(noteData.x+(strumNote.strumSprite.width/2), noteData.y+(strumNote.strumSprite.height/2), noteData.z*0.001), 
            ModchartUtil.defaultFOV*(Math.PI/180), -(strumNote.strumSprite.width/2), -(strumNote.strumSprite.height/2))
