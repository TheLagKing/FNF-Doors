...
import online.GlasshatLogin;
import flixel.FlxBasic;
import flixel.math.FlxRect;
import openfl.ui.MouseCursor;

@:allow(online.GlasshatLogin)
@:allow(backend.UIUtil)
class UISprite extends FlxSprite {
	...
	private inline function get_focused():Bool
		return GlasshatLogin.instance.currentFocus == cast this;

	private inline function set_focused(b:Bool):Bool {
		if (get_focused()) {
			if (!b)
				GlasshatLogin.instance.currentFocus = null;
		} else {
			if (b)
				GlasshatLogin.instance.currentFocus = cast this;
		}
		return b;
	}
	...
	public function updateButtonHandler() {
		GlasshatLogin.instance.updateButtonHandler(this, onHovered);
	}
