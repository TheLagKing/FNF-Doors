...
class UITextBox extends UISliceSprite implements IUIFocusable {
	...
	{
		return label.textField.displayAsPassword;
	}

	private function set_passwordMode(value:Bool):Bool
	{
		label.textField.displayAsPassword = value;
		return value;
	}

	public function new(x:Float, y:Float, text:String = "", width:Int = 320, height:Int = 32, multiline:Bool = false) {
		super(x, y, width, height, "inputbox");

		label = new FlxText(0, 0, width, text);
        label.setFormat(FONT, 32, 0xFFFEDEBF, LEFT, FlxTextBorderStyle.OUTLINE, 0xFF452D25);
