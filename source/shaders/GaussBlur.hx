...
class GaussBlur extends FlxBasic{
    public var shader(default, null):GaussBlurGLSL = new GaussBlurGLSL();

    var iTime:Float = 0;
    
    public var multiplier(default, set):Float = 0.0;
    ...
    public function set_multiplier(v:Float):Float{
        multiplier = v;
        shader.multiplier.value = [v];
        return multiplier;
    }
}
...
class GaussBlurGLSL extends FlxShader{
    @:glFragmentSource('
        ...
        void mainImage()
        {
            ...
            fragColor = BlurH(iChannel0, iResolution.xy, uv, multiplier);
        }
    ')
