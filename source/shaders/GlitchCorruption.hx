...
class GlitchCorruption extends FlxBasic{
    public var shader(default, null):GlitchCorruptionGLSL = new GlitchCorruptionGLSL();
    ...
    public function set_amount(v:Float):Float{
        amount = v;
        shader.Amount.value = [v];
        return amount;
    }
}

class GlitchCorruptionGLSL extends FlxShader{
    @:glFragmentSource('
        #pragma header
        ...
        {
            return flixel_texture2D(sampler, uv - mod(uv, vec2(pixelSize) / openfl_TextureSize.xy));
        }
        ...
        void main()
        {
            ...
            wow = clamp(mod(noise(iTime + openfl_TextureCoordv.xy.y), 1.0), 0.0, 1.0) * 2.0 - 1.0;    
            vec3 finalColor;
            finalColor += distort(bitmap, openfl_TextureCoordv.xy, 8.0);
            gl_FragColor = vec4(finalColor, theAlpha);
        }
    ')
