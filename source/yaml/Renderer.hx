...
class Renderer {
	...
	function detectType(object:Dynamic, explicit:Bool) {
		...
		for (type in typeList) {
			if ((null != type.dumper)
				...
				&& (null == type.dumper.instanceOf
					|| Std.isOfType(object, type.dumper.instanceOf)
	...
	public function kindOf(object:Dynamic) {
		...
		return switch (Type.typeof(object)) {
			...
			case TBool: "boolean";
			case TObject:
				if (Std.isOfType(object, Array)) "array"; else "object";
			case TFunction: "function";
			case TClass(c):
				if (c == String) "string"; else if (c == Array) "array"; else if (c == Bytes) "binary"; else "object";
			case TEnum(_): "enum";
